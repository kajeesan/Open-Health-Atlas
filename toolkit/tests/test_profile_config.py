...
TODAY = datetime.now(ZoneInfo(os.environ["HERMES_TIMEZONE"])).date().isoformat()


# ── profile-set: valid keys ─────────────────────────────────────────────────

def test_profile_set_height_cm(db):
...
def test_profile_set_upserts_same_key(db):
...
def test_phase_set_valid(db, phase):
...
def test_does_not_touch_unrelated_table(db):
    con = sqlite3.connect(db)
