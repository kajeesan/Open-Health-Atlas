...
- `code.di_pattern` = Flask composition through `app.create_app(overrides)` and request configuration; analytical functions receive connections, clocks and `toolkit.hermes_insights.contracts.AdapterContext`. Preserve existing transaction ownership; this does not claim legacy handlers already use explicit dependencies.
